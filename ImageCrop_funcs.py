# #############################################################################
# Collection of functions required for cell cropping.
# CREDIT: These functions are authored by Swarchal
# (https://github.com/Swarchal/NN_cell)
# 
# #############################################################################
...
"""
# ---------------------------------------
# is_outside_img(x, y, img_size, size)
# ---------------------------------------
...
# ---------------------------------------
# nudge_coords(x, y, img_size, size)
# ---------------------------------------
...
# ---------------------------------------
# crop_to_box(x, y, img, size, edge="keep")
# ---------------------------------------
...
# ---------------------------------------
# chop_nuclei(img, size=120, edge="keep", threshold=0.2, **kwargs)
# ---------------------------------------
def chop_nuclei(img, size=120, edge="keep", threshold=0.2, **kwargs):
    """
    ...
    **kwargs : additional arguments to skimage.feature.blob_dog to detect the
    ...
    if img.ndim == 2:
        ...
        # multi channel image, take second channel as nuclei
        nuclei = feature.blob_dog(img[:, :, 1], threshold=threshold, **kwargs)
    else:
        raise ValueError("wrong number of dimensions in img")
...
# ---------------------------------------
# save_chopped(arr, directory, prefix, ext=".png", save_as="img")
# ---------------------------------------
def save_chopped(arr, directory, prefix, ext=".png", save_as="img"):
#def save_chopped(arr, directory, prefix="img", ext=".png", save_as="img"):
    ...
    if save_as == "img":
        for i, img in enumerate(arr, 1):
            img_name = "{}-{}{}".format(prefix, i, ext)
            full_path = os.path.join(os.path.abspath(directory), os.path.basename(img_name))
